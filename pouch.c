...
size_t recv_data_callback(char *ptr, size_t size, size_t nmemb, void *data)
{
	...
	pr->resp.data =
		(char *)realloc(pr->resp.data, pr->resp.size + ptrsize + 1);
...
size_t send_data_callback(void *ptr, size_t size, size_t nmemb, void *data)
{
	...
	if (pr->req.size > 0) {	// only send data if there's data to send
		size_t tocopy =
			(pr->req.size > maxcopysize) ? maxcopysize : pr->req.size;
...
pouch_request *pr_add_param(pouch_request * pr, char *key, char *value)
{
	...
	pr->url = (char *)realloc(pr->url,	// 3: new ? or &, new =, new '\0'
			strlen(pr->url) + 3 +
			sizeof(char) * (strlen(key) + strlen(value)));
...
pouch_request *db_set_revs_limit(pouch_request * pr, char *server, char *db,
		char *revs)
{
	/*
...
pouch_request *doc_get_rev(pouch_request * pr, char *server, char *db, char *id,
		char *rev)
{
	/*
...
pouch_request *doc_get_revs(pouch_request * pr, char *server, char *db,
		char *id)
{
	/*
...
pouch_request *doc_get_info(pouch_request * pr, char *server, char *db,
		char *id)
{
	/*
...
pouch_request *doc_create_id(pouch_request * pr, char *server, char *db,
		char *id, char *data)
{
	/*
...
pouch_request *doc_create(pouch_request * pr, char *server, char *db,
		char *data)
{
	/*
...
pouch_request *doc_get_attachment(pouch_request * pr, char *server, char *db,
		char *id, char *name)
{
	/*
...
pouch_request *doc_copy(pouch_request * pr, char *server, char *db, char *id,
		char *newid, char *revision)
{
	/*
...
pouch_request *doc_delete(pouch_request * pr, char *server, char *db, char *id,
		char *rev)
{
	/*
...
pouch_request *doc_add_attachment(pouch_request * pr, char *server, char *db,
		char *doc, char *filename)
{
	/*
